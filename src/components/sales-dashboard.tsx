...
export function SalesDashboard({ initialData }: SalesDashboardProps) {
  ...
  const filteredData = useMemo(() => {
    return salesData.filter((item) => {
      ...
      const dateInRange = item.timestamp >= fromFilter && item.timestamp <= toFilter
      ...
      const userMatch = !filters.userId || filters.userId === "all" || item.user_id.toString() === filters.userId
  ...
  const uniqueRegions = useMemo(() => [...new Set(salesData.map((item) => item.region))], [salesData])

  const uniqueCategories = useMemo(() => [...new Set(salesData.map((item) => item.category))], [salesData])

  const uniqueUsers = useMemo(() => [...new Set(salesData.map((item) => item.user_id.toString()))], [salesData])
  ...
  const totalItems = useMemo(() => filteredData.reduce((sum, item) => sum + item.items_sold, 0), [filteredData])
  ...
  return (
    <div className="container mx-auto space-y-6">
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-muted-foreground">Analyze your sales data with interactive filters and visualizations.</p>
      </div>
