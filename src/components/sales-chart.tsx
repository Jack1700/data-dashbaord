...
export function SalesChart({ data }: SalesChartProps) {
	...
	const chartData = useMemo(() => {
		...
		const minDate = new Date(
			Math.min(...data.map((item) => item.timestamp.getTime())),
		);
		...
		return days.map((day) => {
			...
			const totalSales = dayData.reduce(
				(sum, item) => sum + item.sales_amount,
				0,
			);
			const totalItems = dayData.reduce(
				(sum, item) => sum + item.items_sold,
				0,
			);
	...
	const regionData = useMemo(() => {
		...
		return regions.map((region) => {
			...
			const totalSales = regionItems.reduce(
				(sum, item) => sum + item.sales_amount,
				0,
			);
			const totalItems = regionItems.reduce(
				(sum, item) => sum + item.items_sold,
				0,
			);
	...
	const categoryData = useMemo(() => {
		...
		return categories.map((category) => {
			...
			const totalSales = categoryItems.reduce(
				(sum, item) => sum + item.sales_amount,
				0,
			);
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>
								Sales amount and items sold over the selected period
							</CardDescription>
						</div>
						<div>
							<Tabs
								value={chartType}
								onValueChange={(value) =>
									setChartType(value as SetStateAction<"area" | "line" | "bar">)
								}
							>
								<TabsList>
